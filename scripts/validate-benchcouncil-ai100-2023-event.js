...
function parseEventSelector(argv) {
    ...
    if (eventIndex === -1 || !argv[eventIndex + 1]) {
        throw new Error('Usage: node scripts/validate-benchcouncil-ai100-2023-event.js --event <order|event-id|work>');
    }
...
function main() {
    ...
    if (event.year !== 2023) failures.push(`event year must be 2023; found ${event.year}`);
