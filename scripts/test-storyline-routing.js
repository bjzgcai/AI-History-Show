    ...
    getPrimaryAudio({
        resources: {
            audios: [
                ...
                {
                    language: 'zh',
                    url: 'https://[REDACTED_OSS_BUCKET].[REDACTED_OSS_ENDPOINT]/audio/ai-history/releases/example.mp3',
                    storage: { provider: 'aliyun-oss' }
                }
            ]
        }
    }).url,
    'https://[REDACTED_OSS_BUCKET].[REDACTED_OSS_ENDPOINT]/audio/ai-history/releases/example.mp3',
        ...
        .filter(
            ({ audio }) =>
                ...
                !String(audio.url || '').startsWith(
                    'https://[REDACTED_OSS_BUCKET].[REDACTED_OSS_ENDPOINT]/audio/ai-history/releases/'
                )
        )
