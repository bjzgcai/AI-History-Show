...
for (const [milestoneId, expected] of Object.entries(expectedEvents)) {
    ...
    assert.equal(images[0], expected.first, `${milestoneId} should lead with its intended detail image`);
...
assert.match(
    indexHtml,
    /function buildUiMediaHtml\(vm, \{ hideVisual = false \} = \{\}\)[\s\S]*?if \(hideVisual\) return ''[\s\S]*?const imageUrl = getUiMediaVisualImage\(vm\)[\s\S]*?if \(!imageUrl\) return ''/,
