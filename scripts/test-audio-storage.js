...
function assertArchiveAudioUsage(root) {
    ...
    for (const eventEntry of fs.readdirSync(eventsRoot, { withFileTypes: true })) {
        ...
        for (const asset of assets.filter((candidate) => candidate.type === 'audio')) {
            ...
            assert.match(
                deliveryUrl,
                /^https:\/\/[REDACTED_OSS_BUCKET]\.oss-cn-beijing\.aliyuncs\.com\/audio\/ai-history\/releases\//,
...
async function main() {
    ...
    try {
        ...
        assert.equal(entries[0].bucket, '[REDACTED_OSS_BUCKET]');
        ...
        assert.equal(config.endpoint, '[REDACTED_OSS_ENDPOINT]');
        assert.equal(config.bucket, '[REDACTED_OSS_BUCKET]');
