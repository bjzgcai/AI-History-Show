...
const state = {
    ...
    figureIdAutoGenerated: '',
    figureIdManuallyEdited: false,
    figureOptions: [],
    eventOptions: [],
    figureListRevision: '',
    figureAssets: [],
...
function nextFigureId(candidate, source) {
    const base = candidate || `figure-${shortTextHash(source || 'new-figure')}`;
    const usedIds = new Set(state.entities.map((entity) => entity.id).filter(Boolean));
...
function updateGeneratedFigureId() {
    if (!state.creatingFigure || state.figureIdManuallyEdited) return;
    const source = elements.figureNameEn.value.trim() || elements.figureNameZh.value.trim();
    if (!source) return;
    const generated = nextFigureId(figureIdSlug(source), source);
    elements.figureId.value = generated;
    state.figureIdAutoGenerated = generated;
}
...
async function refresh() {
    ...
    state.figureIdAutoGenerated = '';
    state.figureIdManuallyEdited = false;
    state.figureAssets = [];
...
function canAutoSaveDraft() {
    ...
    if (state.type === 'figures') {
        if (!/^[a-z0-9][a-z0-9._-]*$/.test(elements.figureId.value.trim())) return false;
        if (!elements.figureNameZh.value.trim() || !elements.figureNameEn.value.trim()) return false;
    }
    return true;
}
...
function createFigure() {
    ...
    renderFigureEvents();
    window.requestAnimationFrame(() => {
        elements.figurePanel.scrollIntoView({ behavior: 'auto', block: 'start' });
        elements.figureId.focus({ preventScroll: true });
    });
...
for (const id of figureFieldIds) {
    elements[id].addEventListener('change', () => {
        ...
        try {
            if (id === 'figureId') {
                state.figureIdManuallyEdited = elements.figureId.value.trim() !== state.figureIdAutoGenerated;
            } else if (id === 'figureNameEn' || id === 'figureNameZh') {
                updateGeneratedFigureId();
            }
            state.document = collectFigureForm();
...
for (const id of ['figureNameEn', 'figureNameZh']) {
    elements[id].addEventListener('input', updateGeneratedFigureId);
}
